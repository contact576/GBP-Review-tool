...
export function LeaderboardRow({ staff, rank }: { staff: StaffMember; rank: number }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2">
      <span className={cn("w-5 text-center text-[14px] font-bold tabular-nums", medal)}>{rank}</span>
      <div className="grid size-8 place-items-center rounded-chip bg-primary-tint text-[12px] font-bold text-primary-dark">
        {staff.avatarInitials}
      </div>
      <div className="min-w-0 flex-1">
        <div className="truncate text-[14px] font-medium text-ink">{staff.displayName}</div>
        <div className="text-[12px] text-faint">{staff.captures} captures</div>
      </div>
      {staff.streakDays > 0 ? (
    ...
    <Link href="/app/benchmark" className="block rounded-card border border-hairline bg-card p-4 transition-colors hover:border-primary/40">
      <div className="flex items-center justify-between">
        <div>
          ...
          {behind > 0 ? (
            <div className="text-[13px] text-sub">{behind} reviews behind the leader</div>
          ) : (
            <div className="text-[13px] text-primary">Leading your area</div>
          )}
        </div>
    ...
    <Link href={href} className="flex items-center gap-3 rounded-btn px-2 py-2 transition-colors hover:bg-primary-wash">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="truncate text-[13px] font-semibold text-ink">{author}</span>
        ...
        <p className="truncate text-[12px] text-sub">{snippet}</p>
      </div>
